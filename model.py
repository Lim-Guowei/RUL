...
def model(filename, result_dir):
    ...
    startTime = time.time()
    if result_dir == "xgbregressor":
        clf = RandomizedSearchCV(mainPipeline, param_distributions=paramsXGBoostRegressor, cv=5, scoring="neg_mean_squared_error", n_jobs=8 ,verbose=2, n_iter=1) # change param_distributions parameter according to fitted model
    
    elif result_dir == "randomforestregressor":
        clf = RandomizedSearchCV(mainPipeline, param_distributions=paramsRandomForestRegressor, cv=5, scoring="neg_mean_squared_error", n_jobs=8 ,verbose=2, n_iter=1) # change param_distributions parameter according to fitted model
    
    elif result_dir == "ridgeregressor":
        clf = RandomizedSearchCV(mainPipeline, param_distributions=paramsRidgeRegressor, cv=5, scoring="neg_mean_squared_error", n_jobs=8 ,verbose=2, n_iter=1) # change param_distributions parameter according to fitted model
